...
var deferred = Q.defer();
    TokenModel.updateMany({ "adminUserId": mongoose.Types.ObjectId(user.id), "type": tokenType }, { "isExpired": "true" })
...
var validateToken = function (token) {
    ...
    try {
        ...
        TokenModel.findOne({ token: token }).populate('adminUserId')
            .then(function (data) {
                if (diffMins <= 0) {
                    if (data) {
                        payload.type = data.type;
                        if (data.isExpired && payload.type === "EmailVerify") {
                            deferred.reject({ "message": Messages.error.BusinessUserEmailVerificationTokenExpired, "type": 1, "emailId": data._doc.adminUserId._doc.emailId });
                        }
                        if (data.isExpired && payload.type === "resetPasswordVerification") {
                            deferred.reject({ "message": Messages.error.resetPasswrodLinkExpired, "type": 1, "emailId": data._doc.adminUserId._doc.emailId });
                        } else {
                            if (data) {
                                payload.type = data.type;
                                ...
                                deferred.resolve({ user: data.adminUserId, payload: payload });
                            } else {
                                deferred.reject({ "message": Messages.error.tokenExpired, "type": 1, "emailId": data._doc.adminUserId._doc.emailId });
                            }
                        }
                    } else {
        ...
        deferred.reject(new Error(Messages.error.InvalidToken));
    }
    return deferred.promise;
};
