...
var createToken = function (user, tokenType, expireTime) {
    ...
    TokenModel.updateMany({ "userId": mongoose.Types.ObjectId(user.id), "type": tokenType }, { "isExpired": "true" })
...
var validateToken = function (token) {
    ...
    try {
        ...
        TokenModel.findOne({ token: token }).populate('userId')
            .then(function (data) {
                if (diffMins <= 0) {
                    payload.type = data.type;
                    if (data.isExpired && payload.type === "EmailVerify") {
                        deferred.reject({ "message": Messages.error.tokenExpired, "type": 1, "emailId": data._doc.userId._doc.emailId });
                    }
                    if (data.isExpired && payload.type === "resetPasswordVerification") {
                        deferred.reject({ "message": Messages.error.resetPasswrodLinkExpired, "type": 1, "emailId": data._doc.userId._doc.emailId });
                    } else {
                        if (data) {
                            payload.type = data.type;
                            ...
                            deferred.resolve({ user: data.userId, payload: payload });
                        } else {
                            deferred.reject({ "message": Messages.error.tokenExpired, "type": 1, "emailId": data._doc.userId._doc.emailId });
                        }
                    }
                ...
                deferred.reject({ "message": Messages.error.InvalidToken });
            })
        //return payload;
    } catch (err) {
        deferred.reject({ "message": Messages.error.InvalidToken });
